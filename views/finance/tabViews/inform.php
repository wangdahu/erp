...
<table class="items">
    <thead>
    <tr>
        <th class="span5">收款销售单</th>
        <th class="span3">未收金额</th>
        <th class="span3">实收金额</th>
        <th>备注</th>
    </tr>
    </thead>
    <tbody>
    ...
    if($listCount){
        ...
        <tr class="<?= ($i+1)%2 ? "odd" : "even"?>">
            <td><?=$customer->salesOrders[$i]->no?></td>
            <td><?=$customer->salesOrders[$i]->notReceivedPrice?></td>
            <td><?=$form->textField($model, "[{$i}]price", array('class' => 'small'))?></td>
            <td>
