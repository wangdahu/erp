        ...
        <div class="item span13">
            <div class="main">
                <span style="padding-left:40px;" id="notice_info">
                    ...
                    if ($is_create) {
                        ?>
                        <input id="notice_info_role" value="role" type="checkbox" name="notice_info_role" checked />
        ...
        $("#start_node_save").click(function(){
            ...
            if(list.length == 0){
                ...
                if(role.length == 0 && user.length == 0 && department.length == 0){
                    ...
                    $("#start_node .role").val(role.join(',')).change();
                    $("#start_node .user").val(user.join(',')).change();
                    $("#start_node .department").val(department.join(',')).change();
                    //节点名称处理 并赋值到隐藏域
                    ...
                    $("#start_node .node_name").val(names).change();
                    var str ='申请人（' +  (names.length > 20 ? (names.substr(0 ,20) + '...') : names) + '）';
                    str += '<br/> 点击修改';
                    $("#start_node a").html(str).attr('title', names);
