...
$gridView = $this->widget('zii.widgets.grid.CGridView', array(
    ...
    'columns'=>array(
        ...
        array('name' => '库存', 'visible' => !$is_view, 'value'=>'Stock::model()->find("storehouse_id=".(empty($data->storehouse_id) ? 0 : $data->storehouse_id)." and product_id=". (empty($data->product_id) ? 0 : $data->product_id))->quantity', 'htmlOptions' => array('class' => 'stock')),
        ...
        array('name' => '操作', 'type'=>'raw', 'value' => 'CHtml::link("&nbsp;", "javascript:", array("class"=>"delete"))', 'headerHtmlOptions'=>array('class'=>'span1'), 'visible'=>!$is_view),
    ),
));
...
houseDropDown = $("#StockOutItem_storehouse_id").clone();
...
function populateData(data){
    ...
    $.each(data, function(i, v){
        ...
        row = $(
            '<tr>'+
                '<td class="product">'+v.name+'<input name="StockOutItem['+v.id+'][product_id]" type="hidden" value="'+v.id+'"></td>'+
                '<td>'+v.no+'</td>'+
                '<td>'+v.brand+'</td>'+
                '<td>'+v.cate+'</td>'+
                '<td>'+v.unit+'</td>'+
                ...
                '<td class="stock"></td>'+
                '<td class="quantity"><input name="StockOutItem['+v.id+'][quantity]" class="span2" min="0" type="text" value="'+v.safe_quantity+'"></td>'+
                '<td class="price"><input name="StockOutItem['+v.id+'][price]" class="span2" type="text" value="'+v.buy_price+'"></td>'+
                '<td class="sub-total">0.00</td>'+
                '<td><a href="javascript:" class="delete">&nbsp;</a></td>'+
            '</tr>');
        row.find('.storehouse').append(
            $("#storehouse_select").clone().show().removeAttr("id").attr("name", "StockOutItem["+v.id+"][storehouse_id]"));
