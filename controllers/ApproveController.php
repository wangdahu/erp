 ...
 *
 */
class ApproveController extends ErpController{
...
/*        if (!class_exists($form_name)){
    ...
    public function actionCreateFlow($form_name) {
        $this->breadcrumbs['设置'] = array('/erp/setting');
        $this->breadcrumbs['审批设置'] = array('/erp/approveflow/index&type=1');
        $this->breadcrumbs[] = '新增流程';
        $flows = ErpFlow::listAllFlowExceptHistory(0);//进销存已有审批流程       参数：1为oa正常审批流程，0为进销存审批流程
    ...
    public function actionUpdateFlow($form_name, $flow_id) {
        $flows = ErpFlow::listAllFlowExceptHistory(0);
    ...
    public function actionInsertFlow(){
        ...
        try {
            ...
            if(isset($_POST['flow_id']) && $_POST['flow_id'] > 0){
                ...
                $flag = ErpFlow::isExistTask($flow_id);
                if(-1004 === $flag){
                    throw new CException('流程不存在');
                }else if(true === $flag){//任务已存在
                    //如果该流程已经存在启动审批的表单 则把原有的流程置为历史
                    ErpFlow::historyFlow($flow_id);
                    //新建流程
                    $flow_id = ErpFlow::insertFlow(array('group_id' => $_POST['group_id'], 'name' => $_POST['flow_name']));
                    //建立表单关联
                    ErpFlow::insertFlowFormRel($flow_id, $_POST['form_name']);
                }else{//任务不存在
                    //直接更新流程名称
                    ErpFlow::updateFlowById(array('name' => $_POST['flow_name']), $flow_id);
                    //删除流程节点以及节点关联对象数据
                    ErpFlow::delFlowData($flow_id);
                }
            }else{
                //新建流程
                $flow_id = ErpFlow::insertFlow(array('group_id' => $_POST['group_id'], 'name' => $_POST['flow_name']));
            ...
            //开始节点数据插入
            ErpFlow::insertNodeData($flow_id, $start_node_data, 1);
            //审批节点数据出入
            foreach ($approve_node_data as $data) {
                ErpFlow::insertNodeData($flow_id, $data, 0);
            }
            //结束节点数据插入
            if(!empty($end_node_data)){
                 ErpFlow::insertNodeData($flow_id, $end_node_data, 2);
            }
            $transaction->commit();
            echo 1;
        } catch (Exception $e) {
    ...
    public function actionCopyFlow($flow_id, $form_name, $group_id){
        if(isset($_POST['new_flow_name']) && !empty($_POST['new_flow_name'])){
            $group_id = ErpFlow::getGroupIdByFlowId($flow_id);
            if($group_id == '0'){
                $res = ErpFlow::copyFlow($flow_id, '', '' , false);
                
                if(is_array($res) && count($res)){
                    ErpFlow::updateFlowById(array('name' => $_POST['new_flow_name']), $res['flow_id']);
                    //更新流程表单对应关系
                    ErpFlow::updateFlowFormRelByFlowId($res['flow_id'], $_POST['current_form_name']);
                    $this->redirect(array('/erp/approve/index', 'form_name' => $form_name));
                }else{
            ...
            $flow = ErpFlow::getFlowById($flow_id);
            if(is_array($flow) && count($flow)){
                $allforms = ErpFlow::listAllFlowExceptHistory();
    ...
    public function actionDeleteFlow($group_id, $flow_id, $form_name) {
        //判断是否存在正在审批的表单
        $flag = ErpFlow::historyFlow($flow_id);
        if(-1004 === $flag){
            throw new CException('流程不存在');
        }else{
            echo json_encode(array('status' => 1, 'location' => $this->createUrl('/erp/approve/index',array('form_name' => $form_name))));
        }
        Yii::app()->end();
    }
    ...
    public function actionCreateNode($user, $role, $department, $div_id, $modify, $is_all,$department_manage) {
        $res = ErpFlow::getSelectedList($user, $role, $department, $department_manage);
    ...
    public function actionCreateEndNode($user,$role,$department,$sms_notice) {
        $res = ErpFlow::getSelectedList($user, $role, $department);
    ...
    public function actionForm($form_id) {
        ...
        //处理表单对应的流程
        $flows = ErpFlow::getFlowsByFormId($form_id);
        $list = array();
        if(is_array($flows) && count($flows)){
            foreach ($flows as $k => $flow) {
                //验证权限
                $auth = ErpFlow::checkStartApproveAuthority($flow['id']);
                //检查流程节点 和 节点关联对象
                $flag = ErpFlow::checkNodeRelate($flow['id']);
                if($auth === true && $flag === true){
                    $approve_str = ErpFlow::getApproveStr($flow['id']);
                    $list[$k]['auth'] = $auth;
                    $list[$k]['flag'] = $flag;
    ...
    public function actionApproved($task_id, $node_id, $node_relate_id, $status, $comment) {
        if (Yii::app()->request->isAjaxRequest) {
            if (!is_numeric($task_id) || !is_numeric($node_id) || empty($node_relate_id) || !is_numeric($status)) {
                ...
                // 审批之前判断事务是否已经完成
                if(ErpFlow::isTaskComplate($task_id)) {
                    ...
                    // 判断事务是否在本节点完成审批
                    if (1 != ErpFlow::getNodeStatus($task_id, $node_id)) {
                        echo json_encode(array('msg' => '当前事务已审批，您此次审批操作无效。', 'flag' => false));
                        Yii::app()->user->setFlash("page_flash", json_encode(array('msg' => '当前事务已审批，您此次审批操作无效。', 'flag' => false, "type" => 'error')));
                        Yii::app()->end();
                    }
                    
                    if (true == ErpFlow::approved($task_id, $node_id, $node_relate_id, $comment, $status)) {
    ...
    public function actionHasRoleWhichFlowEveryNode($flow_id) {
        ...
        $_nodes = ErpFlow::getNormalNodeByFlowId($flow_id);
        foreach ($_nodes as $_node) {
            $_relates = ErpFlow::getRelateByNodeId($_node['id']);
            if (empty($_relates)) {
                ...
                foreach ($_relates as $_relate) {
                    $_roles = ErpFlow::getRelateUserList($_relate['type'], $_relate['relate_id'], $_user_id);
                    if (empty($_roles)) {
                        $_messages = array("msg" => $_node['name'].'审批环节无具体审批人，此流程不可用，请联系管理员进行设置', 'flag' => false);
                        echo json_encode($_messages);
                        Yii::app()->end();
        ...
        try {
            ...
            if (Yii::app()->request->isPostRequest) {
                ...
                if ($_request_param && "4" == $status) {
                    ...
                    //$_form_info = ErpFlow::getFormInfoByFlowId($flow_id);
                    ...
                    try {
                        ...
                        // 发送消息
                        ErpFlow::sendInMessage($_to_users, $_msg, $class_name, $task_id);
                        if ($_sms_notice) {
                            ErpFlow::sendSms($_to_users, $_msg);
                        }
...
//                    // 找到这个环节下所有的审批人
//                    $_last_approve_users = ErpFlow::getNodeUserIdByNodeId($_last_node_id, $task_id);
//                    // 看当前用户是否属于这个环节
...
//                        
//                        // 发送消息
//                        ErpFlow::sendInMessage($_to_users, $_msg, $task_id);
//                        ErpFlow::sendInMessage($_approve_to_user, $_approve_receive_msg, $task_id);
//                        if ($_sms_notice) {
//                            ErpFlow::sendSms($_to_users, $_msg);
//                        }
//                        // 重整页面
...
//                }
    ...
    protected function _getApproveUsers($task_id, $need_tag = '0') {
        $_flow_id = ErpFlow::getFlowIdByTask($task_id);
        switch ($need_tag) {
        case 1:
            // 当前审批环节的审批人(正在流转)
            $_node_id = ErpFlow::getCurrentNode($task_id);
            break;
        case 2:
            // 第一批审批人
            $_node_id = ErpFlow::getFirstNode($_flow_id);
            break;
        case 3:
            // 最后一批审批人
        ...
        if (!is_array($_node_id)) {
            $_approve_users = ErpFlow::getNodeUserIdByNodeId($_node_id, $task_id);
        } else {
            $_approve_users = array();
            foreach ($_node_id AS $_node) {
                $_users = ErpFlow::getNodeUserIdByNodeId($_node['node_id'], $task_id);
